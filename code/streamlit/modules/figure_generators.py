...
# colours for lab vs non-lab (blue / orange — never red+green together)
C_LAB    = PAIR_LAB_NONLAB[0]   # "#0072B2" blue
C_NONLAB = PAIR_LAB_NONLAB[1]   # "#E69F00" orange
C_MALE   = PAIR_MALE_FEMALE[0]  # "#0072B2" blue
C_FEMALE = PAIR_MALE_FEMALE[1]  # "#CC79A7" pink-purple
...
def make_fig1(nl_df: pd.DataFrame, site_df: pd.DataFrame | None = None) -> tuple:
    ...
    (a) Butterfly pyramid by age-band × gender
    (b) Stacked bar by age-band × gender (population share)
    """
    ...
    # --- Panel a: Butterfly population pyramid ---
    bands = AGE_BANDS
    n_bands = len(bands)
    ...
    # --- Panel b: stacked bars by age band (absolute % share) ---
...
def make_fig2(nl_df: pd.DataFrame) -> tuple:
    ...
    (a) Butterfly pyramid (lab sub-cohort with cholesterol)
    (b) Stacked bars by diabetes status × SBP band
    """
    ...
    # --- Panel a: butterfly for lab cohort ---
    bands   = AGE_BANDS
    n_bands = len(bands)
    y_pos   = np.arange(n_bands)
    ...
    # --- Panel b: stacked by diabetes × SBP band ---
    if "has_diabetes" in df_lab.columns and "sbp_band" in df_lab.columns:
        ...
        # separator line between diabetic and non-diabetic groups
...
def make_fig3(paired_df: pd.DataFrame) -> tuple:
    ...
    # colour: diagonal=steelblue, above-diagonal (under)=orange, below=grey
    ...
    # --- Panel b: Sankey-style alluvial bars ---
    ...
    # draw left bars (non-lab) and right bars (lab)
    bar_w = 0.08
    nl_tops = {}
    lb_tops = {}
    nl_cum, lb_cum = 0, 0
    ...
    # draw flow ribbons between categories using cross-tabulation
...
def make_fig4(nl_df: pd.DataFrame) -> tuple:
    ...
    (a) Prevalence of ≥10% and ≥20% by age band with Wilson CIs
    (b) Prevalence of high risk across continuous 1–30% thresholds
    """
    df = _ensure_age_band(nl_df).copy()
    ...
    # --- Panel a: age escalation with Wilson CIs ---
    bands   = AGE_BANDS
    y_10, lo_10, hi_10 = [], [], []
    y_20, lo_20, hi_20 = [], [], []
    ns = []
    ...
    # --- Panel b: prevalence by threshold ---
...
def make_fig6(nl_df: pd.DataFrame, site_df: pd.DataFrame | None = None) -> tuple:
    ...
    Sites ranked by prevalence; colour-coded by location type.
    Pooled random-effects mean shown as dashed vertical line.
    """
    df = _ensure_age_band(nl_df).copy()
    df["risk_nonlab"] = pd.to_numeric(df["risk_nonlab"], errors="coerce")
    ...
    # Location type colours (Okabe-Ito, no red-green pair)
    ...
    # Figure height scales with site count
    ...
    pooled = site_agg_df["prev"].mean()   # simple mean as reference
