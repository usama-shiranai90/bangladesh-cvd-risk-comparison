    ...
    with tab2:
        ...
        with c2:
            ...
            if 'urban_rural' in df_cohort.columns:
                ...
                if risk_model_opt == "Both (Compare)":
                    ...
                    ur_nl.columns = ['urban_rural', 'risk_cat', 'percent']
                    ur_nl['Model'] = 'Non-Lab'
        ...
        if 'gender' in df_cohort.columns:
            ...
            if risk_model_opt == "Both (Compare)":
                ...
                model_hatches = {'Non-Lab': '', 'Lab': '///'}
                x = np.arange(len(sorted(ag_comb['age_band'].unique().astype(str))))
        ...
        if risk_model_opt == "Both (Compare)":
            ...
            df_cohort['underestimates'] = (df_cohort['risk_diff'] < -2).astype(int)
        ...
        **Comprehensive validation of non-laboratory-based CVD risk assessment against laboratory-based reference standard.**
        ...
        if datasets and 'paired' in datasets and datasets['paired'] is not None:
            ...
            if 'age' not in df_paired.columns or 'risk_lab' not in df_paired.columns or 'risk_nonlab' not in df_paired.columns:
                ...
                df_paired['misclass_10_under'] = df_paired['lab_ge10'] & ~df_paired['nonlab_ge10']
                df_paired['misclass_10_over'] = ~df_paired['lab_ge10'] & df_paired['nonlab_ge10']
                
                df_paired['lab_ge20'] = df_paired['risk_lab'] >= 20
                df_paired['nonlab_ge20'] = df_paired['risk_nonlab'] >= 20
                df_paired['misclass_20_under'] = df_paired['lab_ge20'] & ~df_paired['nonlab_ge20']
                df_paired['misclass_20_over'] = ~df_paired['lab_ge20'] & df_paired['nonlab_ge20']
                ...
                df_paired['mets_partial_positive'] = df_paired['mets_partial_score'] >= 2
                ...
                def plot_risk_sankey(data, title_suffix=""):
                    ...
                    COLOR_LAB_BAR = "#0C5DA5"
                    COLOR_NONLAB_BAR = "#845B97"
                ...
                if target_col in df_paired.columns:
                    ...
                    ct = ct.sort_values("Underestimation", ascending=True)
                ...
                p_new = df_paired['risk_lab']
                p_ref = df_paired['risk_nonlab']
                ...
                cat_new_bin = (p_new >= nri_thresh).astype(int)
                
                nonlab_agree_high = ((cat_ref_bin==1) & (y_true==1)).sum()
                nonlab_agree_low = ((cat_ref_bin==0) & (y_true==0)).sum()
                lab_correct_high = ((cat_ref_bin==0) & (y_true==1)).sum()
                lab_correct_low = ((cat_ref_bin==1) & (y_true==0)).sum()
                ...
                if strat_col_resid in df_paired.columns:
                    ...
                    for i, grp in enumerate(groups):
                        ...
                        if len(sub_df) > 0:
                            ...
                            chi2, p, dof, exp = stats.chi2_contingency(obs + 0.0001)
                ...
                **Advanced analysis of how multiple cardiovascular risk factors interact** to influence:
                ...
                with int_tab4:
                    ...
                    for rf_n in range(6):
                        ...
                        if n >= 10:
                            ...
                            underest = (lab_high & ~nonlab_high).sum()
                            overest = (~lab_high & nonlab_high).sum()
                ...
                categorical_vars = [
                    ...
                    ('bp_category', 'Hypertension (≥140/90)', None)
                ]
                ...
                for gly_group in gly_order:
                    ...
                    if n_g > 0:
                        ...
                        underest = ((df_g['risk_lab'] >= 10) & (df_g['risk_nonlab'] < 10)).sum()
                        overest = ((df_g['risk_lab'] < 10) & (df_g['risk_nonlab'] >= 10)).sum()
                ...
                **Analyzing patterns across multiple visits** for patients with repeated measurements.
                ...
                if 'visit_num' in df_paired.columns and 'total_visits' in df_paired.columns:
                    ...
                    if 'visit_year' in df_paired.columns and df_paired['visit_year'].nunique() > 1:
                        ...
                        risk_by_year = risk_by_year[risk_by_year['N'] >= 10]
